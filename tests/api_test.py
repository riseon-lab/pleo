...
check("asset attached to job history", r.status_code == 200, r.text)
q = c.get("/api/queue", headers=H).json()
check("queue history exposes asset_id", any(j.get("asset_id") == asset_id for j in q["history"]), str(q["history"][:2]))
r = c.delete(f"/api/jobs/{job_id}/history", headers=H)
check("history entry removable", r.status_code == 200, r.text)
check("history entry gone", not any(j["id"] == job_id for j in c.get("/api/queue", headers=H).json()["history"]))
r = c.post("/api/queue/clear", headers=H)
check("queue history clearable", r.status_code == 200, r.text)
...
# --- validation ---
r = c.post("/api/generate", headers=H, json={**gen, "steps": 1, "width": 1080, "height": 1920})
check("FHD auto-snapped to latent grid (1080 -> 1072)",
      r.status_code == 200 and r.json()["job"]["width"] == 1072 and r.json()["job"]["height"] == 1920, r.text[:200])
r = c.post("/api/generate", headers=H, json={**gen, "steps": 1, "width": 250})
check("odd width auto-rounded (250 -> 256)", r.status_code == 200 and r.json()["job"]["width"] == 256, r.text[:200])
